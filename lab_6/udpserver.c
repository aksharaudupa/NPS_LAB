...
void str_echo(int connfd)
{
    ...
    n=recvfrom(connfd, buffer, bufsize, 0,(struct sockaddr *)&cli,&size);
	     sendto(connfd,buffer,n,0,(struct sockaddr *)&cli,size);
        printf("%s\n",buffer);
             //printf("%d n",n);
	//  if(n<0)
    //         goto again;
...
int main()
{
  ...
  if ((listenfd = socket(AF_INET,SOCK_DGRAM,0)) > 0) //sockfd
